...
const LUCIDE_SVGS = {
  ...
  'sparkles': '<path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"/><path d="M20 3v4"/><path d="M22 5h-4"/><path d="M4 17v2"/><path d="M5 18H3"/>',
...
async function build() {
  ...
  // Update all dist HTML files to reference bundle.css instead of the two separate files
  const CSS_TWO_TAGS = '<link rel="stylesheet" href="colors_and_type.css?v=2">\n<link rel="stylesheet" href="styles.css?v=3">';
  const CSS_BUNDLE_TAG = '<link rel="stylesheet" href="bundle.css?v=3">';
  fs.readdirSync(distDir).forEach(file => {
    if (!file.endsWith('.html')) return;
    const p = path.join(distDir, file);
    const content = fs.readFileSync(p, 'utf8');
    if (content.includes(CSS_TWO_TAGS)) {
      fs.writeFileSync(p, content.replace(CSS_TWO_TAGS, CSS_BUNDLE_TAG));
    }
  });

  // ── Optimization 2: Inline Lucide SVGs and remove CDN script ───────────
  console.log('Inlining Lucide SVGs...');
  const LUCIDE_CDN_SCRIPT = '<script src="https://unpkg.com/lucide@0.484.0/dist/umd/lucide.min.js" defer></script>';
  // Pages that must keep the CDN (they use Lucide dynamically at runtime)
  const LUCIDE_KEEP_CDN = new Set(['reservar.html', 'cotizacion.html']);
  ...
  fs.readdirSync(distDir).forEach(file => {
    ...
    if (!LUCIDE_KEEP_CDN.has(file)) {
      // Remove CDN script tag
      content = content.replace(LUCIDE_CDN_SCRIPT, '');
